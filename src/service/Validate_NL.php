...
class Validate_NL
{
    const VALIDATE_NL_PHONENUMBER_TYPE_ANY = 0;
    const VALIDATE_NL_PHONENUMBER_TYPE_NORMAL = 1;
    const VALIDATE_NL_PHONENUMBER_TYPE_MOBILE = 2;
     ...
     */
    public static function phoneNumber($number, $type = self::VALIDATE_NL_PHONENUMBER_TYPE_ANY)
    {
        $result = false;
        ...
        if (preg_match("/^[+0-9]{9,}$/", $number)) {
            ...
            if (strlen($number) >= 9) {
                switch ($type) {
                    case self::VALIDATE_NL_PHONENUMBER_TYPE_ANY:
                        //we have a 9 digit numeric number.
                        $result = true;
                        break;
                    case self::VALIDATE_NL_PHONENUMBER_TYPE_NORMAL:
                        //normal phonenumbers don't begin with 6 (00316, +316
                        //and 06 are reserved for mobile numbers)
                        if ((int)$number[0] != 6)
                            $result = true;
                        break;
                    case self::VALIDATE_NL_PHONENUMBER_TYPE_MOBILE:
     ...
     */
    public static function bankAccountNumber($number)
    {
